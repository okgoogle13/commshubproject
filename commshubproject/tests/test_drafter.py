...
def _make_drafter():
    """Instantiate Drafter with a dummy key, patching anthropic.Anthropic so no real connection is made."""
    with patch("src.drafter.anthropic.Anthropic"):
...
def test_drafter_returns_three_modes():
    ...
    with patch.object(drafter.client.messages, "create", return_value=_mock_response(json_out)):
...
def test_drafter_strips_json_fences():
    ...
    with patch.object(drafter.client.messages, "create", return_value=_mock_response(fenced)):
...
def test_drafter_handles_parse_error_gracefully():
    drafter = _make_drafter()
    with patch.object(drafter.client.messages, "create", return_value=_mock_response("not json at all")):
        result = drafter.draft_reply("test", silence_days=0)
...
def test_drafter_passes_silence_days_in_prompt():
    ...
    def capture(**kwargs):
        captured.append(str(kwargs.get("messages", [])))
        return _mock_response('{"minimal": "x", "honest": "y", "practical_reentry": "z"}')

    with patch.object(drafter.client.messages, "create", side_effect=capture):
